...
export default function BlogContents(blogId: any) {
    ...
    // links.map((url) => {
    //     async () => {
    //         const result = await unfurl(url.url);
    //         temps.push(result);
    //     }
    // })
    ...
    //     const cardLinks = cardDatas.map((data: any) => data.open_graph.url);
    //     function link(text: any) {
    //         return text.indexOf(node.attribs?.href) != -1;
    //     }
    //     if (node.name === "a" && //タグがa
